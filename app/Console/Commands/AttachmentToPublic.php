...
class AttachmentToPublic extends Command
{
    protected $signature = 'backup:attachment-to-public {id}';
    ...
    public function handle()
    {
        ...
        if (!Storage::disk('ftp')->exists($localPath)) {
            $this->error("File in ftp server does not exist: {$localPath}");
            return Command::FAILURE;
        }else{

            // if the file is found , proceed 
            $contents = Storage::disk('ftp')->get($localPath);


            // check if the file is already on ftp, to not make double file upload 
            if (Storage::disk('public')->exists($localPath)) {
                $this->error("File in public server already exist: {$localPath}");
                return Command::FAILURE;
            }else{
                // if it does not exists continue uploading it 
                Storage::disk('public')->put($localPath, $contents);

                $this->info("Backup sent to Public server successfully.");
