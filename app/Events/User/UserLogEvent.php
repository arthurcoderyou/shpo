...
class UserLogEvent  implements ShouldBroadcast, ShouldQueue

{
    ...
    {    
        // Always broadcast to the user-scoped user channel
        $channels = [
        ...
        if (!is_null($this->modelId)) { 
            // $channels = [];
            $channels[] = new PrivateChannel('user.'. $this->modelId);
        }
