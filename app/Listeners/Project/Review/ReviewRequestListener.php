...
use App\Events\Project\Review\ReviewRequest;
use App\Mail\Project\Review\ReviewRequestMail;
use App\Notifications\Project\Review\ReviewRequestNotification;
...
class ReviewRequestListener implements ShouldQueue
{
    /**
     ...
     * Handle the event.
     */
    public function handle(ReviewRequest $event): void
    {
        ...
        if($event->sendMail){
            ...
            try {
                Mail::to($user->email)->queue(
                    new ReviewRequestMail( $project_reviewer, $project,$reviewUrl)
                );
            } catch (\Throwable $e) {
                // Log the error without interrupting the flow
                Log::error('Failed to dispatch ReviewRequestMail mail: ' . $e->getMessage(), [
        ...
        if($event->sendNotification){
            ...
            try { 
                Notification::send($user, new ReviewRequestNotification($project_reviewer, $project));
            } catch (\Throwable $e) {
                // Log the error without interrupting the flow
                Log::error('Failed to dispatch ReviewRequestNotification notification: ' . $e->getMessage(), [ 
                    'project_id' => $project->id,
                    'trace' => $e->getTraceAsString(),
                ]);
            }
