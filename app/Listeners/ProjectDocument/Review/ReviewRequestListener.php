...
class ReviewRequestListener implements ShouldQueue
{
    ...
    public function handle(ReviewRequest $event): void
    {
        ...
        if($event->sendMail){
            ...
            try {
                Mail::to($user->email)->queue(
                    new ReviewRequestMail( $project_reviewer, $project, $project_document,$reviewUrl)
                );
            } catch (\Throwable $e) {
        ...
        if($event->sendNotification){
            ...
            try { 
                Notification::send($user, new ReviewRequestNotification($project_reviewer, $project, $project_document));
            } catch (\Throwable $e) {
                // Log the error without interrupting the flow
