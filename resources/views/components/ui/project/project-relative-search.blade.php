...
>
    ...
    <input
        ...
        type="{{ $type }}"
        x-model="search" 
        x-on:focus="if (search.trim().length > 0) open = true"
        x-on:input="open = search.trim().length > 0"
        autocomplete="off"
        placeholder="{{ $placeholder }}"
        {{ $attributes->whereStartsWith('wire:model') }}
        class="mt-1 block w-full rounded-lg border border-slate-300 px-3 py-2  text-md focus:border-indigo-500 focus:ring-indigo-500"
    >

    {{-- Dropdown --}}
    <div
        x-show="open && search.trim().length > 0"
        x-transition
        x-cloak
        class="absolute z-20 mt-1 max-h-64 max-w-full w-full overflow-y-auto rounded-lg border border-slate-200 bg-white shadow-lg"
    >
