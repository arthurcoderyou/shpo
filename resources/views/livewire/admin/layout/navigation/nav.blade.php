...
new class extends Component {
    ...
    public function icon(string $name): string
    {
        return match ($name) {
            ...
            // 🔹 Projects (Own)
            'projects-own' => '<svg class="w-5 h-5 text-slate-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
            ...
            // 🔹 Settings (FontAwesome style)
            'settings' => '<svg class="w-5 h-5 text-slate-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
            ...
            // 🔹 Activity Logs (FontAwesome history/clock style)
            'activity-logs' => '<svg class="w-5 h-5 text-slate-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
            ...
            // 🔹 Project Settings (FontAwesome fa-tools original SVG)
            'project-settings' => '<svg class="w-5 h-5 text-slate-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
            ...
            // 🔹 File Manager (FontAwesome folder style)
            'file-manager' => '<svg class="w-5 h-5 text-slate-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512">
...
<nav class="{{ $navWrap }}">
                                                    ...
                                                    @if (!empty($child['children']))
                                                        <ul class="ml-5 mt-1 space-y-1">
                                                            @foreach ($child['children'] as $gchild)
                                                                @php $activeG = $this->isActiveTree($gchild); @endphp
                                                                <li>
                                                                    <a href="{{ route($gchild['route']) }}"
                                                                        wire:navigate
                                                                       class="{{ $this->linkClasses($activeG, 3) }}">
                                                                        {{-- {{ $gchild['label'] }} --}}
                                                                        <span>{{ $gchild['label'] }}</span>
                                                                        @if ($this->showCount($gchild))
                                                                            <span class="{{ $this->badgeBase }} {{ $activeG ? $this->badgeActive : $this->badgeMuted }}">
                                                                                {{ $this->formatCount((int) $gchild['count']) }}
                                                                            </span>
                                                                        @endif
                                                                    </a>
                                                                </li>
    ...
    {{-- Optional footer (only for mobile in your example) --}}
    {{-- @if ($variant === 'mobile')
        <div class="px-4 py-4 border-t mt-4">
