...
<div class="max-w-[85rem] px-4 py-6 sm:px-6 lg:px-8  mx-auto">
    ...
    <div>
        ...
        @php
            // 1 = Initial Review, 2 = Document Review, 3 = Final Review
        ...
        <div class="w-full mx-auto mb-2">
            ...
            <div class="bg-white shadow rounded-2xl p-6">
                <h2 class="text-lg font-semibold text-slate-800">Project Review Flow</h2>
                ...
                <div class="absolute left-[12.5%] top-6 h-0.5 -z-10 bg-indigo-600 transition-all duration-300"
                    style="right: {{ [1=>'87.5%', 2=>'50%', 3=>'12.5%'][$currentStage] ?? '87.5%' }}"></div>

                <!-- Step 1: Initial Review -->
                <li class="flex flex-col items-center text-center w-1/3">
                    <div class="flex items-center justify-center w-12 h-12 rounded-full ring-2 
                                {{ $currentStage > 1 ? 'bg-indigo-600 ring-indigo-600 text-white' : ($currentStage === 1 ? 'bg-white ring-indigo-600 text-indigo-600' : 'bg-white ring-slate-300 text-slate-400') }}">
                    @if ($currentStage > 1)
                        <!-- check -->
                    ...
                    <div class="mt-3">
                    <div class="text-sm font-medium {{ $currentStage >= 1 ? 'text-slate-900' : 'text-slate-400' }}">Initial Review</div>
                    <p class="text-xs {{ $currentStage === 1 ? 'text-indigo-600' : 'text-slate-500' }}">
                ...
                <li class="flex flex-col items-center text-center w-1/3">
                    <div class="flex items-center justify-center w-12 h-12 rounded-full ring-2
                                {{ $currentStage > 2 ? 'bg-indigo-600 ring-indigo-600 text-white' : ($currentStage === 2 ? 'bg-white ring-indigo-600 text-indigo-600' : 'bg-white ring-slate-300 text-slate-400') }}">
                    @if ($currentStage > 2)
                        <svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                        </svg>
                    ...
                    <div class="mt-3">
                    <div class="text-sm font-medium {{ $currentStage >= 2 ? 'text-slate-900' : 'text-slate-400' }}">Document Review</div>
                    <p class="text-xs {{ $currentStage === 2 ? 'text-indigo-600' : 'text-slate-500' }}">
                ...
                <li class="flex flex-col items-center text-center w-1/3">
                    <div class="flex items-center justify-center w-12 h-12 rounded-full ring-2
                                {{ $currentStage > 3 ? 'bg-indigo-600 ring-indigo-600 text-white' : ($currentStage === 3 ? 'bg-white ring-indigo-600 text-indigo-600' : 'bg-white ring-slate-300 text-slate-400') }}">
                    @if ($currentStage > 3)
                        <svg class="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                        </svg>
                    ...
                    <div class="mt-3">
                    <div class="text-sm font-medium {{ $currentStage >= 3 ? 'text-slate-900' : 'text-slate-400' }}">Final Review</div>
                    <p class="text-xs {{ $currentStage === 3 ? 'text-indigo-600' : 'text-slate-500' }}">
                        {{ $currentStage === 3 ? 'In progress' : ($currentStage > 3 ? 'Completed' : 'Pending') }}
                    </p>
                    </div>
                </li>
                </ol>
                ...
                <li class="relative pl-10 pb-6">
                    ...
                    <span class="absolute left-2 top-1 w-6 h-6 rounded-full ring-2 
                    {{ $currentStage > 1 ? 'bg-indigo-600 ring-indigo-600 text-white' : ($currentStage === 1 ? 'bg-white ring-indigo-600 text-indigo-600' : 'bg-white ring-slate-300 text-slate-400') }}
                    flex items-center justify-center">
                    @if ($currentStage > 1)
                        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg>
                    @else <span class="text-xs font-semibold">1</span> @endif
                    </span>
                    <div>
                    <div class="text-sm font-medium {{ $currentStage >= 1 ? 'text-slate-900' : 'text-slate-400' }}">Initial Review</div>
                    <p class="text-xs {{ $currentStage === 1 ? 'text-indigo-600' : 'text-slate-500' }}">
                ...
                <li class="relative pl-10 pb-6">
                    ...
                    @if ($currentStage > 2)
                        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg>
                    @else <span class="text-xs font-semibold">2</span> @endif
                    </span>
                    <div>
                    <div class="text-sm font-medium {{ $currentStage >= 2 ? 'text-slate-900' : 'text-slate-400' }}">Document Review</div>
                    <p class="text-xs {{ $currentStage === 2 ? 'text-indigo-600' : 'text-slate-500' }}">
                ...
                <li class="relative pl-10">
                    ...
                    @if ($currentStage > 3)
                        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg>
                    @else <span class="text-xs font-semibold">3</span> @endif
                    </span>
                    <div>
                    <div class="text-sm font-medium {{ $currentStage >= 3 ? 'text-slate-900' : 'text-slate-400' }}">Final Review</div>
                    <p class="text-xs {{ $currentStage === 3 ? 'text-indigo-600' : 'text-slate-500' }}">
        ...
        <div class="bg-white rounded-xl shadow mb-2">
            ...
            <div class="  p-4">
                ...
                <div class="sm:col-span-12">
                    ...
                    Add Project Reviewer for "{{ $project->name ?? null }}"
                    </h2>
                </div>
                <!-- End Col -->
                ...
                <div class="grid grid-cols-12 gap-x-2  ">
                    ...
                    <div class="space-y-2 col-span-12 sm:col-span-6">
                     ...
                     {{-- <div class="space-y-2 col-span-12 "> --}}
                    <div class="space-y-2 col-span-12 sm:col-span-6">
                        <label for="review_status" class="inline-block text-sm font-medium text-gray-800 mt-2.5 ">
                            Review Status
                        </label>

                        <select autofocus autocomplete="review_status"
                            ...
                            <tr>
                                ...
                                <td class="w-10 whitespace-nowrap flex flex-row items-center">
                                    ...
                                    <div class="flex flex-col">
                                        <button {{ $reviewer['order']  == 1 ? 'disabled' : '' }} type="button" 
                                            wire:click="updateOrder( {{ $key }},{{ $reviewer['order'] }},'move_up',{{ $reviewer['project_document_id'] ?? 0 }}, '{{ $reviewer['reviewer_type'] }}' )" 
                                        ...
                                        <button {{ $reviewer['order'] == $lastOrder ? 'disabled' : '' }} type="button" 
                                            wire:click="updateOrder( {{ $key }},{{ $reviewer['order'] }},'move_down',{{ $reviewer['project_document_id'] ?? 0 }} , '{{ $reviewer['reviewer_type'] }}' )" 
                                            class="p-1 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none  " >
                                            <div class="hs-tooltip flex">
                                    ...
                                    <div class="px-2 py-2">
                                        <div class="flex items-center gap-x-3">
                                            <div class="grow">
                                                ...
                                                @else 
                                                    <span class="block text-sm text-gray-500 ">{{ __('Open Admin Review') }}</span>
                                                @endif
                                ...
                                <td class="size-auto whitespace-nowrap">
                                    <div class="px-2 py-2">
                                        <div class="flex items-center gap-x-3">
                                            <div class="grow">
                                                <span class="block text-sm text-gray-500 ">{{ $reviewer['status'] == true ? "Yes" : "No" }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="size-auto whitespace-nowrap">
                                    <div class="px-2 py-2">
                                        <div class="flex items-center gap-x-3">
                                            <div class="grow">
                                                <span class="block text-sm text-gray-500 uppercase">{{ $reviewer['review_status']   }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </td>
