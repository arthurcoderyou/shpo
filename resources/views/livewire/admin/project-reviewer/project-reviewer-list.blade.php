        ...
        <div class="p-1.5 min-w-full inline-block align-middle">
            ...
            <div class="px-3 py-2 grid gap-3 md:flex md:justify-between md:items-center border-b border-gray-200 dark:border-neutral-700">
                ...
                <h2 class="text-xl font-semibold text-gray-800 dark:text-neutral-200">
                    <a class="hover:text-blue-500" href="{{ route('project.show',['project' => $project->id]) }}">"{{ $project->name }}"</a>  Reviewers
                </h2>
                <p class="text-sm text-gray-600 dark:text-neutral-400">
                    Listing of reviewers for the project
                </p>
                </div>
                ...
                <div class="inline-flex gap-x-2">
                    ...
                    @if( Auth::user()->hasRole('Admin') || Auth::user()->can('project reviewer edit') || Auth::user()->hasRole('DSI God Admin') )  
                        <button title="This is to apply the reviewer list here for the current project {{ $project->name }}"
                            onclick="confirm('Are you sure, you want to apply this reviewer order list to the project?') || event.stopImmediatePropagation()"
                            wire:click.prevent="notify_to_all" 
                            ...
                            <tr>
                                @if(Auth::user()->hasRole('Admin') || Auth::user()->can('project reviewer edit') || Auth::user()->hasRole('DSI God Admin'))
                                    <td class="w-10 whitespace-nowrap flex flex-row items-center">
                                        ...
                                        <div class="flex flex-col">
                                            <button {{ $reviewer->order  == 1 ? 'disabled' : '' }} type="button" wire:click="updateOrder({{ $reviewer->id }}, {{ $reviewer->order }},'move_up')" class="p-1 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600" >
                                            ...
                                            <button {{ $reviewer->order == $lastOrder ? 'disabled' : '' }} type="button" wire:click="updateOrder({{ $reviewer->id }}, {{ $reviewer->order }},'move_down')" class="p-1 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600" >
                                                <div class="hs-tooltip flex">
                                ...
                                <td class="size-auto whitespace-nowrap">
                                    <div class="px-2 py-2">
                                        <div class="flex items-center gap-x-3">
                                            <div class="grow">
                                                <span class="block text-sm text-gray-500 dark:text-neutral-500">{{ $reviewer->user ? $reviewer->user->name : '' }}</span>
                                                <span class="block text-sm text-gray-500 dark:text-neutral-500">{{ $reviewer->user ? $reviewer->user->email : '' }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </td>

                                <td class="size-auto whitespace-nowrap text-center">
                                    <div class="px-2 py-2">
                                        <div class="flex items-center gap-x-3">
                                            <div class="grow">
                                                <span class="block text-sm text-gray-500 dark:text-neutral-500">{{ $reviewer->status ? 'Yes' : 'No' }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="size-auto whitespace-nowrap text-center">
                                    <div class="px-2 py-2">
                                        <div class="flex items-center gap-x-3">
                                            <div class="grow">
                                                <span class="block text-sm text-gray-500 dark:text-neutral-500">{{ $reviewer->order }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </td>
